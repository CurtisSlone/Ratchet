...
- **The action chain (3.2, 3.4.5).** A chain of signed, hash-linked step records is structurally an in-toto [4] / SLSA [5] attestation chain, the software-supply-chain provenance pattern, applied to an agent's actions instead of a build pipeline.
...
**2.2.3 Determinism in the trust path.** The model may propose. A deterministic policy decides. Enforcement is deterministic. No non-deterministic component sits anywhere a wrong answer authorizes an action. This is the hard line that makes the system defensible.
...
- **Input hash.** A content hash of what the action rests on, so the system can prove what it acted against without storing the raw, often sensitive, values.
- **Decision reference.** The signed token from the deterministic PDP: allow or deny, and which policy fired.
- **Replay hash.** A deterministic, identity-free hash over intent, context, and outcome. Same action and same posture produce the same hash. Drift detection and tamper detection in a single field.
- **Prior reference.** The hash of the preceding event in the same action chain, so a multi-step action is an ordered, hash-linked sequence and not a loose set of records (3.4.5).
...
**3.3.4 Event substrate.** The streaming bus (Kafka [14] or equivalent) carrying signed action-events. It provides durability, replay, and decoupling. The architecture's contribution is that the substrate carries signed facts, not bare messages, so durability and replay become tamper-proof history. A note on regime: per-event signing, Merkle appends, and proof generation suit high-value, lower-frequency action events, the events that authorize action, not a high-throughput telemetry firehose. The cost of proof is justified where an action must be defended, not on every tick of the bus.

### 3.4 The Verifiable Log

The verifiable log is the audit trail that is the system, not a log written beside it. An append-only log is not enough on its own. The log's operator is an actor like any other, and Zero Trust does not exempt it. The log must let any party prove two things without trusting whoever runs it: that a given action is in the record, and that the record has only ever grown. The Merkle-tree transparency-log pattern standardized in RFC 6962 provides both.

**3.4.1 Structure and the signed tree head.** Events are the leaves of a Merkle tree. The root hash commits to the entire set of events: change any leaf and the root changes. The log periodically signs the current root together with the tree size, producing a **signed tree head (STH)**, a public commitment that says "at this size, the log contains exactly this set of events." The STH is what everything else is proven against.

**3.4.2 Inclusion proofs.** To prove a specific event is in the log, the log returns an audit path: the O(log n) sibling hashes needed to recompute the root from that leaf. A verifier recomputes the root and checks it against the signed tree head. The event is proven present without the verifier trusting the operator and without the operator revealing the rest of the tree. The proof is not zero-knowledge: the verifier sees the event and its path, not the unrelated leaves.

**3.4.3 Consistency proofs.** An inclusion proof alone is not sufficient, and this is the subtlety most "append-only log" claims miss. An inclusion proof only shows membership under whatever root the operator presents now. A dishonest operator could quietly rebuild the tree, dropping or altering a past event, and still produce a valid inclusion proof against the rewritten version. A **consistency proof** closes that hole: given two signed tree heads, it proves the earlier one is a prefix of the later one, that the log only appended, and that nothing in between was deleted, reordered, or modified. Consistency proofs are what make "append-only" something you can verify rather than something you are asked to believe.

**3.4.4 Chained and witnessed tree heads.** Two mechanisms harden the timeline of roots. First, each event binds the hash of the prior event (the prior reference in 3.2), so the event sequence is itself an ordered hash chain independent of the tree, which gives cheap ordering and gap detection. Second, each signed tree head references the previous root and size, and the heads are gossiped to independent witnesses. Witnessing defeats **equivocation**: a log that tries to show one history to one verifier and a different history to another is caught the moment two parties compare the roots they were given. Without witnessing, a forked history is possible; with it, it is not. One honest limit: inside a single organization the witnesses likely share a trust root and an administrator, so intra-org witnessing weakens the equivocation defense. Cross-organization or independent third-party witnessing is what actually closes it.

**3.4.5 Integrity of the action chain.** A multi-step agent action is not one event but a chain: sense, reason, decide, act, prove (3.7). Each step is recorded as a signed event, and each step binds the hash of the step before it, so the whole chain, from the triggering input through the model's proposal, the deterministic decision, the enforced action, and the outcome, is a single hash-linked sequence. Altering the model's proposal after the fact, or the decision that followed it, breaks every subsequent hash in the chain. The completed chain's head is then a leaf in the log's Merkle tree, so the properties above apply to the chain as a whole: it can be proven present (inclusion), proven unaltered relative to the rest of history (consistency), and proven to belong to one non-equivocating timeline (witnessing). The upgrade is concrete. An agent run that is otherwise a directory of plain step records, a start record, an ordered set of step outputs, and an outcome, mutable and deletable with no trace, becomes an ordered, signed, individually verifiable chain whose tampering cannot go undetected.

**3.4.6 What "tamper-proof" means here.** The log is tamper-proof in the precise sense that matters for an audit trail: any alteration, deletion, or reordering is detectable and provable by a party that trusts none of the actors, and with independent witnessing, undetectable tampering is computationally infeasible. The guarantee is detection and non-repudiation, not a claim that bytes on disk cannot be changed. For a record whose only job is to be trusted, that is the stronger property: you do not need to stop a dishonest operator from trying, you need to make trying useless. The proofs are not zero-knowledge; a verifier sees the data it verifies. The cryptography is standard (RFC 6962 [3], as deployed in Sigstore's Rekor [6]) and deliberately unoriginal. A fair question is how this differs from in-toto [4] or SLSA [5], which already sign hash-linked step metadata over a pipeline. In mechanism, it does not. The difference is the target and the contract: the steps are an agent's runtime actions rather than build stages, and the same signed contract covers human actors too (1.5). Applying the supply-chain provenance pattern to the agent's own action chain is the contribution, not the cryptography.
...
**3.6.5 Event verification at ingest.** Before any workflow runs, the triggering event's signature is verified. The system acts only on events cryptographically confirmed to originate from a known producer. This single gate defeats event injection and replay.

### 3.7 Operational Overview (the provable loop)

The loop has five steps. Each produces a signed event, and each event binds the one before it, so a completed run is the hash-linked action chain of 3.4.5.

**Step 1: Sense.** A signed event arrives on the substrate. The control plane verifies its signature and replay hash before anything else. An unverifiable event is rejected and logged. This is the only thing the system trusts to start work.
...
**A2A.** Agent-to-agent messages ride the substrate as signed events. One agent cannot drive another on an unverifiable message; A2A handoffs are verified like any other event, which prevents an injected or impersonated agent message from propagating through the chain.

**The substrate.** Kafka or an equivalent carries the signed events and provides replay. Because the events are signed facts, the substrate's natural durability becomes a tamper-proof history rather than a queue that forgets.
...
*A working reference implementation demonstrates the core of this design: trusted-source prompt construction, a model that proposes while a deterministic check decides, and action chains recorded as ordered step sequences. The cryptographic verifiable log of section 3.4, signing and consistency proofs included, is the planned next layer.*
