...
An instance is data Ratchet loads - the host code never changes per domain. It is one `ratchet.json`
config plus four buckets. Copy `examples\template` (a complete, self-documented skeleton with a
working example chain) and replace its contents.
...
{
  "name": "my-icm",
  "domain": "one line: what this instance is for",
...
shortlist, with vectors cached per-instance in `.index/`. Semantic re-rank is most worth it at
...
reference instances do: keep each kb **small and focused**, leave the `embed` seat on for curated
...
The host runs the command with the **instance root as the working directory** (so `tools/...` resolves),
...
session focus. The C# reference instance scaffolds a project there with `new_project`, builds it up
