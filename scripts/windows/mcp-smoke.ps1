...
param([string] $Dir)
# This script lives in scripts\windows\: the sibling RatchetBox repo is three levels up, and the
# ratchet.cmd launcher now lives under bins\windows-amd64\ (both relative to $PSScriptRoot).
if (-not $Dir) { $Dir = Join-Path $PSScriptRoot "..\..\..\RatchetBox\Windows\dotnet4-x" }   # pass -Dir to override
...
$launcher = Join-Path $PSScriptRoot "..\..\bins\windows-amd64\ratchet.cmd"
